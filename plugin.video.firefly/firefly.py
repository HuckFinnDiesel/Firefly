...
if action is None:
	...
	if cache._find_cache_version():
		run = 'true'
	if run == 'true':
		control.execute('RunPlugin(plugin://plugin.video.firefly/?action=cleanSettings)')
...
####################################################
	...
	if params.get('opensettings') == 'true':
		control.openSettings('0.0', "plugin.video.firefly")

elif action == 'openscrapersSettings':
	control.openSettings('0.0', 'script.module.openscrapers')
	if params.get('opensettings') == 'true':
		control.openSettings(query, "plugin.video.firefly")
